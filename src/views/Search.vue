<template>
    <div class="searchTop">
        ...
        <input type="text" placeholder="陈奕迅" v-model="data.searchKey" @keydown.enter="enterKey" />
    </div>
     <div class="searchHistory">
        <span class="searchSpan">历史</span>
        <span v-for="item in data.keyWorldList" :key="item" class="spanKey" @click="searchHistory(item)">
            {{ item }}
        </span>
...
async function enterKey(){
    // 结构数据
    let { keyWorldList, searchKey } = data
    if(searchKey !== '') {
        // 向头部插入数据
        keyWorldList.unshift(searchKey)
        // 去重
        data.keyWorldList = [...new Set(keyWorldList)]
        // 固定长度
        if(keyWorldList.length > 10) {
            console.log("length");
            data.keyWorldList.splice(keyWorldList.length - 1,1)
        }
        // 持久化存储用户搜索数据
        localStorage.setItem("keyWorldList", JSON.stringify(data.keyWorldList))
        // 发送请求获取用户搜索
